...
Class Menu_model extends CI_Model
{
       ...
       public function buildTreeMainFull($cats, $parent_id, &$k = 0) {  
               ...
               $count = 1; foreach($cats[$parent_id] as $cat) {
                  ...
                  /* if($cat->category_id == 142 || $cat->category_id == 143 || $cat->category_id == 144){
                       if($cat->category_id == 142){
                         $link1 = site_url('contests/category/'.$cat->alias);
                       }
                        if($cat->category_id == 143){
                         $link1 = site_url('tenders/category/'.$cat->alias);
                       }
                       if($cat->category_id == 144){
                         $link1 = site_url('contracts/category/'.$cat->alias);
                       }
                  ...
                  // }
                   ...
                   if ($cat->category_id == 0) {
                       ...
                       $link = '<li class="'.$nav_sub.'"><a data-'.$toggleClass.'"'.$cat->options.'" class="'.$nav_link.' '.$no_link.'" href="'.$link1.'">'.@$title[LANG].$icon_menu.'</a>';
                       }elseif($cat->option_2){
              ...
              //  if($cat->category_id == 2){
               ...
               // }else{
                ...
                if ($cat->category_id == 0) {
                    ...
                    if($cat->as_menu == 1) {
                        if($cat->options){
                        $link = '<li class=" dropdown-submenu nav-item"><a data-alias="'.$cat->alias.'" class="dropdown-item dropdown-toggle '.$no_link.' '.$nav_link.'" href="'.$link1.'">'.@$title[LANG].$icon_menu.'</a>';
                         }elseif($cat->option_2){
                        $link = '<li class=" dropdown-submenu nav-item"><a data-alias="'.$cat->option_2.'" class="dropdown-item dropdown-toggle '.$no_link.' '.$nav_link.'" href="'.$link1.'">'.@$title[LANG].$icon_menu.'</a>';
                        } else {
                          $link = '<li class="dropdown-submenu nav-item"><a data-alias="'.$cat->alias.'" class="dropdown-item dropdown-toggle '.$no_link.' '.$nav_link.'" href="'.$link1.'">'.@$title[LANG].$icon_menu.'</a>';
                        }
                    } else {
                    ...
                    $link = '<li class="dropdown nav-item"><a data-alias="'.$cat->options.'" class="dropdown-toggle1 '.$no_link.' '.$nav_link.'" href="'.$link1.'">'.@$title[LANG].$icon_menu.'</a>';
                    }elseif($cat->option_2){
                    $link = '<li class="dropdown nav-item"><a data-alias="'.$cat->options_2.'" class="dropdown-toggle1 '.$no_link.' '.$nav_link.'" href="'.$link1.'">'.@$title[LANG].$icon_menu.'</a>';
                    } else {
                      $link = '<li class="dropdown nav-item"><a data-alias="'.$cat->alias.'" class="dropdown-toggle1  '.$no_link.' '.$nav_link.'" href="'.$link1.'">'.@$title[LANG].$icon_menu.'</a>';
                    }
                    }
                   // }
                }  else {
                ...
                $icon_menu1 = ($sub) ? '<i class="fa fa-angle-right" aria-hidden="true"></i>' : '' ;
                            ...
                            if($cat->as_menu == 1) {
                    if($cat->options){
                    $link = '<li class="dropdown dropdown-submenu"><a class="dropdown-item dropdown-toggle1" href="'.$link1.'">'.@$title[LANG].$icon_menu1.'</a>';
