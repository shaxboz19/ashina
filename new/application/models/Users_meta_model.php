...
Class Users_meta_model extends CI_Model
{
	...
	{
		return $this->db->get_where('users_meta',array('post_id'=> $id))->result();
	}
   ...
   public function save($data, $id = false)
	{
     ...
     public function save_user_accept_f1($data, $id = false)
	{
		if ($id)
		{
    ...
    public function save_user_accept_f2($data, $id = false)
	{
		if ($id)
		{
			$this->db->where('user_id_add', $id)
					 ->update('users_meta', $data);
		}	
	}
  	...
  	public function delete($id, $type)
	{
    	...
    	$this->db->where('post_id', $id)
              ->where_in('type_u', $type)
		         ->delete('users_meta');
	}
  
  	public function delete_type_main($id, $type)
	{
    	...
    	public function delete_users($id, $type)
	{
    ...
    public function delete_users_2($id, $type)
	{
  ...
  public function get_users_meta($args = null)
	{
		$defaults = array(		
			'limit' => 10000,
			'post_id' => '',
      ...
      'order'  => 'DESC',        
		...
		$this->db->select('users_meta.*')				 
				 ->where('users_meta.post_id', $q['post_id'])
				 ->group_by('users_meta.id');

        if(!empty($q['status']))
            $this->db->where('users_meta.status_u', $q['status_u']);
            if(!empty($q['type_u']))
            $this->db->like('users_meta.type_u', $q['type_u']);
      ...
      if ( !empty($q['sort_order']) )
		...
		return $this->db->get('users_meta', $q['limit'], $q['offset'])->result();
	}
  
    public function get_users_meta_all($args = null)
	{
		$defaults = array(		
			'limit' => 10000,
			'post_id' => '',
      ...
      'offset' => 0,     
		...
		$this->db->select('users_meta.*')				 
				 ->where('users_meta.type_u', $q['type_u'])
				 ->group_by('users_meta.id');
  ...
  public function get_users_info($args = null)
	{
		$defaults = array(		
			'limit' => 10000,
			'post_id' => '',
      ...
      'order'  => 'DESC',        
		...
		$this->db->select('users_meta.*')				 
				 ->where('users_meta.user_id', $q['user_id'])
				 ->group_by('users_meta.id');
  ...
  public function get_users_info_2($args = null)
	{
		$defaults = array(		
			'limit' => 10000,
			'post_id' => '',
      ...
      'order'  => 'DESC',        
		...
		$this->db->select('users_meta.*')				 
				 ->where('users_meta.user_id_add', $q['user_id_add'])
				 ->group_by('users_meta.id');

        if(!empty($q['status_f']))
            $this->db->where_in('users_meta.status_f', $q['status_f']);
            if(!empty($q['type_u']))
            $this->db->like('users_meta.type_u', $q['type_u']);
