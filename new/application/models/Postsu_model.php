...
Class Postsu_model extends CI_Model
{
   ...
   public function save_option_1($data_option_1, $id)
	{
       ...
       public function save1($data, $id = FALSE)
	{
		if ($id)
		{
			$this->db->where('id', $id)
  ...
  public function get_posts_portfolio($group, $category=FALSE, $limit=200, $offset=0)
	{
		$this->db->select('posts_u.*, media_u.url, categories.title as category,  t2.alias as parent_alias, categories.alias as category_alias')
				 ...
				 ->where('posts_u.user_id', $this->session->userdata('user_id'))
				 ->order_by('id DESC')
				 ->group_by('posts_u.id');
       ...
       'id' => '',
		...
		if ( !empty($q['category_id']) )
			$this->db->where_in('posts_u.category_id', $q['category_id']);
            	...
            	if ( !empty($q['user_id']) )
			$this->db->where_in('posts_u.user_id', $q['user_id']);
  ...
  public function get_posts_region($args = null)
	{
		$defaults = array(
			'group' => 'video',
			'category_ids' => array(),     
        ...
        if(!empty($q['status']))
            $this->db->where('posts_u.status', $q['status']);
        if(!empty($q['spec']))
            $this->db->where_in('posts_u.spec', $q['spec']);
       /* else
            $this->db->where('posts_u.status !=', 'draft');*/
...
return $this->db->count_all_results();
  ...
  public function get_posts_count_admin($group = 'pages') {
   ...
   public function get_posts_count_region($group = 'pages', $id) {
		$sql = "SELECT
                  COUNT(*) AS `count`
                FROM posts_u AS p
                WHERE p.group = '".$group."'
