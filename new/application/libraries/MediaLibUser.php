...
Class MediaLibUser
{
	...
	{
		$media = $this->ci->db->get_where('media_u', array('id'=>$id))->row();

		if ($media)
		{
			@unlink( "./uploads/{$media->category}/{$media->url}" );

			$this->ci->db->delete('media_u', array('id'=>$id));
		}
	}

	public function set_main($media_id)
	{
		$media = $this->ci->db->get_where('media_u', array('id'=>$media_id))->row();
