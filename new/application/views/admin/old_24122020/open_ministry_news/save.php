<?=form_open_multipart('', array('class'=>'form-horizontal my_form'))?>
    ...
    */
    ?>
    <div class="control-group">
		<label class="control-label" for="focusedInput">Подкатегория</label>
		<div class="controls">
			<select name="category_id2" class="input-xlarge focused">
                <option value="0">Нет</option>
                <?php 
               $sub = getOptionsData(array('group' => 'open_ministry_category', 'status' => 'active', 'category_id' => @$post->category_id,'media' => 'inactive')); 
                ?>
                <? foreach($sub as $item): ?>
                <option value="<?=$item->id?>" <?php echo @$item->id == @$post->category_id2 ?'selected="selected"':'';?>> <?=_t($item->title)?> </option>
                <? endforeach; ?>
            </select>
		</div>
    </div>
