...
">Сохранить</button>          
           ...
           <!-- <td>
            <a href="<?= site_url('admin/pages/index/regions_news/'.$post->id) ?>" class="btn btn-info">Новости</a>
            </td>
            <td>
            <a href="<?= site_url('admin/pages/index/regions_manage/'.$post->id) ?>" class="btn btn-info">Руководство</a>
            </td>-->
            <td><?=char_lim(_t($post->title), 90)?></td>
           <td><?=char_lim(_t($post->category_title), 90)?></td>
                 <td>
              <div class="btn-group">
