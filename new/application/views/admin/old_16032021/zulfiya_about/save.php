<?=form_open_multipart('', array('class'=>'form-horizontal my_form'))?>
	...
	/*
		...
		<div class="controls">
			<select name="category_id" class="input-xlarge focused">
                <option value="0">Нет</option>
                <? foreach($nc as $item): ?>
                <option value="<?=$item->id?>" <?php echo @$post->category_id == $item->id?'selected="selected"':'';?>><?=_t($item->title, 'ru')?></option>
                <? endforeach; ?>
            </select>
		</div>
    </div>
    <div class="control-group">
		<label class="control-label" for="focusedInput">На гл.слайдер</label>
		<div class="controls">
    ...
    */
    ...
    <div class="control-group" style="display:none;">
		...
		<div class="controls">
                ...
                <input id="alias1211313" name="alias" class="span3" type="text" value="<?=set_value('alias', $post->alias)?>">
