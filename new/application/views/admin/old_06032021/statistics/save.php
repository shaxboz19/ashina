<?=form_open_multipart('', array('class'=>'form-horizontal my_form'))?>
            ...
            <? $i=1; foreach ($settings['languages']->value as $key => $val): ?>
                <div class="tab-pane  <?=($i==1) ? 'active' : ''?>" id="tab<?=$i?>">
                    ...
                    <div class="control-group">
                        ...
                        <div class="controls">
                            <input id="title123123" name="title[<?=$key?>]" class="input-xlarge focused titles123123" type="text" value="<?=set_value('title['.$key.']', _t(@$post->title, $key))?>">
                        </div>
                    </div>
   ...
   <!-- <div class="control-group">
	<label class="control-label" for="focusedInput">Внутреняя ссылка</label>
	<div class="controls">
