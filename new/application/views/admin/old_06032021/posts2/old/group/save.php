        ...
        foreach ($settings['languages']->value as $key => $val) : ?>
            <div class="tab-pane  <?= ($i == 1) ? 'active' : '' ?>" id="tab<?= $i ?>">
                ...
                <div class="control-group">
                    ...
                    <div class="controls">
                        <input id="title" name="title[<?= $key ?>]" class="titles input-xlarge focused" type="text" value="<?= set_value('title', _t(@$post->title, $key)) ?>">
                    </div>
                </div>
                ...
                <?php if ($sel == 'news') { ?>
                    <div class="control-group">
                        <label class="control-label" for="focusedInput">Автор</label>
                        <div class="controls">
                            <input id="option_4" name="option_4[<?= $key ?>]" class="span3" type="text" value="<?= set_value('option_4', _t(@$post->option_4, $key)) ?>">
                        </div>
                    </div>
                    <div class="control-group">
                        ...
                        <div class="controls">
                            <textarea style=" width:100%; height:300px;" name="short_content[<?= $key ?>]"><?= set_value('short_content[' . $key . ']', _t(@$post->short_content, $key)) ?></textarea>
                        </div>
                    </div>
                <?php } ?>
                    ...
                    <div class="control-group">
                        ...
                        <div class="controls">
                            <input name="short_content[<?= $key ?>]" class="moxiecut" type="text" value="<?= set_value('short_content[' . $key . ']', _t(@$post->short_content, $key)) ?>">
                        </div>
                    </div>
                ...
                <?php } elseif ($sel == 'career_list') { ?>
                    <div class="control-group">
                        <label class="control-label" for="focusedInput">Должностные обязанности</label>
                        <div class="controls">
                            <textarea name="short_content[<?= $key ?>]" class="moxiecut"><?= set_value('short_content', _t(@$post->short_content, $key)) ?></textarea>
                        </div>
                    </div>
                    <div class="control-group">
                        <label class="control-label" for="focusedInput">Требования</label>
                        <div class="controls">
                            <textarea name="option_4[<?= $key ?>]" class="moxiecut"><?= set_value('option_4', _t(@$post->option_4, $key)) ?></textarea>
                        </div>
                    </div>
                    <div class="control-group">
                        <label class="control-label" for="focusedInput">Условия работы</label>
                        <div class="controls">
                            <textarea name="content[<?= $key ?>]" class="moxiecut"><?= set_value('content', _t(@$post->content, $key)) ?></textarea>
                        </div>
                    </div>
                    <div class="control-group">
                        <label class="control-label" for="focusedInput">Контакты</label>
                        <div class="controls">
                            <textarea name="content_html[<?= $key ?>]" class="moxiecut"><?= set_value('content_html', _t(@$post->content_html, $key)) ?></textarea>
                        </div>
                    </div>

                <?php } else { ?>
                    <?php if ($sel == 'news' || $sel == 'about' || $sel == 'docs' || $sel == 'regional_news' || $sel == 'projects' || $sel == 'subject' || $sel == 'invest') { ?>
                        <div class="control-group">
...
<?php if ($sel == 'territorial_category' || $sel == 'territorial') { ?>
    <div class="control-group">
        <label class="control-label" for="focusedInput">Email</label>
        <div class="controls">
            <input name="option_2" class="input-xlarge focused" type="email" value="<?= set_value('option_2', @$post->option_2) ?>">
        </div>
    </div>
    <div class="control-group">
        <label class="control-label" for="focusedInput">Телефон</label>
        <div class="controls">
            <input name="option_3" class="input-xlarge focused" type="tel" value="<?= set_value('option_3', @$post->option_3) ?>">
        </div>
    </div>
    <div class="control-group">
        <label class="control-label" for="focusedInput">Факс: </label>
        <div class="controls">
            <input name="option_1" class="input-xlarge focused" type="tel" value="<?= set_value('option_1', @$post->option_1) ?>">
        </div>
    </div>
<?php } ?>
<?php if ($sel == 'news') { ?>
    ...
    <div class="control-group">
        ...
        <div class="controls">
            <select name="lang_status" class="input-xlarge focused">
                ...
                <option value="oz" <?php echo @$post->lang_status == 'oz' ? 'selected="selected"' : ''; ?>> Узбекский(кириллица)</option>
            </select>
        </div>
    </div>
<?php } ?>
...
<?php } else { ?>
    <div class="control-group" <?php if ($sel == 'release' or $sel == 'doc' or $sel == 'news') { ?>style="display: none;" <?php } ?>>
    ...
    <div class="control-group">
        ...
        <div class="controls">
            <?php if ($post->alias) { ?>
                <input name="alias" class="span3" type="text" readonly="" value="<?= set_value('alias', $post->alias) ?>">
            <?php } else { ?>
                <input id="alias" name="alias" class="validate[required,ajax[check_alias2]] span3" type="text" value="<?= set_value('alias', $post->alias) ?>">
            <?php } ?>
        </div>
    </div>
