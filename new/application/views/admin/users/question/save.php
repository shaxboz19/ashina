<?=form_open_multipart('', array('class'=>'form-horizontal my_form'))?>
    ...
    <div class="control-group">
     ...
     <div class="control-group">
         <label class="control-label" for="focusedInput">Одобрено</label>
         <div class="controls">
            <select name="approved" class="input-xlarge focused">
                <option value="active" <?php echo @$post->approved == 'active'?'selected="selected"':'';?>><?=lang('active')?></option>
                <option value="inactive" <?php echo @$post->approved == 'inactive'?'selected="selected"':'';?>> <?=lang('inactive')?> </option>
            </select>
        </div>
    </div>
